...
class PickupService {
    ...
    public function createPickupService($data)
    {
        ...
        foreach ($data['items'] as $key => $value) {
            $validator = Validator::make($value, [
                'unit_count'        => 'bail|required|numeric|digits_between:1,255',
                'name'              => 'bail|required|string|max:255',
                'weight'            => 'bail|required|numeric|min:0|digits_between:1,255',
                'volume'            => 'bail|required|numeric|min:0|digits_between:1,255',
                'service_id'        => 'bail|present|nullable',
                'route_price_id'    => 'bail|required|numeric',
        ...
        try {
            $price = $this->billRepository->calculatePriceRepo($items, $route, $promo, false, $cost);
        } catch (Exception $e) {
            DB::rollback();
            Log::info($e->getMessage());
            Log::error($e);
    ...
    public function getReadyToPickupService($data = [])
    {
        $validator = Validator::make($data, [
            ...
            'name' => 'bail|present',
            'city' => 'bail|present',
            'district' => 'bail|present',
            'village' => 'bail|present',
            'picktime' => 'bail|present',
            'sort' => 'bail|present',
            'number' => 'bail|present'
        ]);
    ...
    public function createPickupAdminService($data = [])
    {
        ...
        if ($data['form']['newCustomer'] == true) {
            ...
            try {
                $isExistEmail = $this->userRepository->getByEmail($data['customer']['email']);
            } catch (Exception $e) {
                DB::rollback();
                Log::info($e->getMessage());
                Log::error($e);
            ...
            if (!$isExistEmail) {
                ...
                $passed = false;
                $email = $data['customer']['email'];
                throw new InvalidArgumentException("Pengguna dengan email $email sudah terdaftar, silahkan gunakan email lain");
            }

            try {
                $isExistPhone = $this->userRepository->getByPhone($data['customer']['phone']);
            } catch (Exception $e) {
                DB::rollback();
                Log::info($e->getMessage());
                Log::error($e);
            ...
            if (!$isExistPhone) {
                ...
                $passed = false;
                $phone = $data['customer']['phone'];
                throw new InvalidArgumentException("Pengguna dengan nomor telepon $phone sudah terdaftar, silahkan gunakan nomor telepon lain");
            }
        ...
        // ESTIMATE PRICE
        try {
            $price = $this->billRepository->calculatePriceRepo($items, $route, $promo, false, $data['cost']);
        } catch (Exception $e) {
            DB::rollback();
            Log::info($e->getMessage());
            Log::error($e);
        ...
        if ($data['isDrop']) {
            ...
            // SAVE PICKUP PLAN
            try {
                $result = $this->pickupPlanRepository->savePickupPlanRepo([$pickup['id']], 0, $data['userId'], 0);
            } catch (Exception $e) {
                DB::rollback();
                Log::info($e->getMessage());
                Log::error($e);
            ...
            try {
                $bill = $this->billRepository->calculatePriceRepo($items, $route, $promo, true, $data['cost']);
            } catch (Exception $e) {
                DB::rollback();
                Log::info($e->getMessage());
                Log::error($e);
            ...
            // SAVE INSURANCE AND TAX
            $cost = [
                'pickupId' => $pickup['id'],
                'taxRate' => $price->total_tax_rate,
                'taxAmount' => $price->total_tax_amount,
                'insuranceAmount' => $price->total_insurance_amount,
                'amountWithTax' => $price->total_price_with_tax,
                'amountWithInsurance' => $price->total_price_with_insurance,
                'amountWithTaxInsurance' => $price->total_price_with_tax_insurance,
            ];
            try {
                $this->costRepository->saveInsuranceAndTax($cost);
            } catch (Exception $e) {
                DB::rollback();
                Log::info($e->getMessage());
                Log::error($e);
                throw new InvalidArgumentException('Gagal menyimpan biaya asuransi dan pajak');
            }
            // END INSURANCE AND TAX
        }
    ...
    public function getReadyToShipmentService($data = [])
    {
        $validator = Validator::make($data, [
            ...
            'page'      => 'bail|numeric',
            'name'      => 'bail|present|nullable',
            'city'      => 'bail|present|nullable',
            'id'        => 'bail|present|nullable',
            'district'  => 'bail|present|nullable',
            'village'   => 'bail|present|nullable',
            'picktime'  => 'bail|present|nullable',
            'sort'      => 'bail|present|nullable',
            'number'    => 'bail|present|nullable',
            'branchId'  => 'bail|present|numeric',
            'isTransit' => 'bail|present|nullable',
        ]);
    ...
    public function getListShipmentPlanService($data = [])
    {
        $validator = Validator::make($data, [
            ...
            'status'        => 'bail|present',
            'driver'        => 'bail|present',
            'licenseNumber' => 'bail|present',
            'vehicleType'   => 'bail|present',
            'sort'          => 'bail|present',
            'branchId'      => 'bail|present|numeric',
            'number'        => 'bail|present',
    ...
    public function editPickupAdminService($data = [])
    {
        ...
        DB::beginTransaction();
        // CHECK PICKUP HAVE POP OR NOT
        if (!$data['isDrop']) {
            try {
                $this->pickupRepository->checkPickupHavePOPByPickup($data['form']['id']);
            } catch (Exception $e) {
                DB::rollback();
                Log::info($e->getMessage());
                Log::error($e);
            ...
            try {
                $this->pickupRepository->checkPickupHaveShipmentPlanByPickup($data['form']['id']);
            } catch (Exception $e) {
                DB::rollback();
                Log::info($e->getMessage());
                Log::error($e);
        ...
        // foreach ($items as $key => $answer) {
        // 	unset($items[$key]['service']);
        //     unset($items[$key]['route_price']);
        //     unset($items[$key]['with_minimum']);
        // }
        try {
    ...
    public function cancelDropService($data = [])
    {
        ...
        // JIKA BUKAN ADMIN, DELETE BIASA
        if ($user->role_id !== 9) {
            // CANCEL DROP
            try {
                $drop = $this->pickupRepository->cancelDropRepo($data['pickupId'], $data['userId']);
            } catch (Exception $e) {
                DB::rollback();
                Log::info($e->getMessage());
                Log::error($e);
            ...
            try {
                $payload = [
                    'pickupPlanId' => $drop['pickup_plan_id']
                ];
                $pickupPlan = $this->pickupPlanRepository->cancelPickupPlanRepo($payload);
            } catch (Exception $e) {
                DB::rollback();
                Log::info($e->getMessage());
                Log::error($e);
            ...
            // DELETE POD
            try {
                $this->podRepository->deletePodRepo($data);
            } catch (Exception $e) {
                DB::rollback();
                Log::info($e->getMessage());
                Log::error($e);
            ...
            // DELETE INCOMING / TRANSIT
            try {
                $this->transitRepository->deleteTransitRepo($data['pickupId'], $data['userId']);
            } catch (Exception $e) {
                DB::rollback();
                Log::info($e->getMessage());
                Log::error($e);
            ...
            if ($shipmentPlanId !== null) {
                try {
                    $shipmentPlan = $this->shipmentPlanRepository->getShipmentPlanById($shipmentPlanId);
                } catch (Exception $e) {
                    DB::rollback();
                    Log::info($e->getMessage());
                    Log::error($e);
                ...
                if (count($shipmentPlan->pickups) == 1) {
                    // DELETE SHIPMENT PLAN
                    try {
                        $shipmentPlan->deleted_by = $data['userId'];
                        $shipmentPlan->save();
                        $shipmentPlan->delete();
                    } catch (Exception $e) {
                        DB::rollback();
                ...
                // UNASSIGN SHIPMENT PLAN FROM PICKUP
                try {
            ...
            // DELETE POP
            try {
                $this->popRepository->deleteByPickupId($data['pickupId'], $data['userId']);
            } catch (Exception $e) {
                DB::rollback();
                Log::info($e->getMessage());
                Log::error($e);
            ...
            // DELETE PICKUP PLAN
            $pickupPlanId = $pickup->pickup_plan_id;
            if ($pickupPlanId !== null) {
                try {
                    $pickupPlan = $this->pickupPlanRepository->getById($pickupPlanId);
                } catch (Exception $e) {
                    DB::rollback();
                    Log::info($e->getMessage());
                    Log::error($e);
                ...
                // UNASSIGN PICKUP PLAN FROM PICKUP
                try {
                    $pickup->pickup_plan_id = null;
                    $pickup->updated_by = $data['userId'];
                    $pickup->save();
                    $pickup->fresh();
                } catch (Exception $e) {
        ...
        // SET COST AMOUNT TO ZERO
        try {
            $this->costRepository->setAmountToZeroByPickupRepo($data['pickupId']);
        } catch (Exception $e) {
            DB::rollback();
            Log::info($e->getMessage());
            Log::error($e);
     ...
     */
    public function getPrintedPickupOrderService($data = [])
    {
        try {
            $result = $this->pickupRepository->getPrintedPickupRepo($data['pickupNumber']);
        } catch (Exception $e) {
            Log::info($e->getMessage());
            throw new InvalidArgumentException('Gagal mendapatkan data pickup');
        }
    ...
    /**
     * mendapatkan total order terbuat berdasarkan rentang filter dan cabang di dashboard
     */
    public function getTotalOrderCreatedService($data = [])
    {
        ...
        try {
            $result = $this->pickupRepository->getTotalOrderCreatedRepo($data['filter']);
        } catch (Exception $e) {
            Log::info($e->getMessage());
            Log::error($e);
            throw new InvalidArgumentException('Gagal mendapatkan total order terbuat');
        }
    ...
    /**
     * mendapatkan total order yang dibatalkan berdasarkan rentang filter dan cabang di dashboard
     */
    public function getTotalOrderCancelledService($data = [])
    {
        $validator = Validator::make($data['filter'], [
            'startDate' => 'bail|required',
            'endDate'   => 'bail|required',
        ...
        try {
            $result = $this->pickupRepository->getTotalOrderCanceledRepo($data['filter']);
        } catch (Exception $e) {
            Log::info($e->getMessage());
            Log::error($e);
            throw new InvalidArgumentException('Gagal mendapatkan total order yang dibatalkan');
        }
     ...
     * cancel pickup order
     */
    public function cancelPickupService($data = [])
    {
        $validator = Validator::make($data, [
            'userId' => 'bail|required',
            'pickupId' => 'bail|required'
        ...
        if ($user->role_id !== 9) {
            ...
            // CANCEL PICKUP
            try {
                $result = $this->pickupRepository->cancelPickupRepo($data);
            } catch (Exception $e) {
                DB::rollback();
                Log::info($e->getMessage());
                Log::error($e);
    ...
    /**
     * mendapatkan performa armada berdasarkan rentang filter dan cabang di dashboard
     */
    public function getFleetPerformanceService($data = [])
    {
        $validator = Validator::make($data['filter'], [
            'startDate' => 'bail|required',
            'endDate'   => 'bail|required',
        ...
        try {
            $result = $this->pickupRepository->getFleetPerformanceRepo($data['filter']);
        } catch (Exception $e) {
            Log::info($e->getMessage());
            Log::error($e);
            throw new InvalidArgumentException('Gagal mendapatkan kinerja armada');
        }
    ...
    /**
     * mendapatkan data order perbulan berdasarkan cabang di dashboard
     */
    public function getOrderPerMonthService($data = [])
    {
        $validator = Validator::make($data['filter'], [
            'branch'    => 'bail|required|array'
        ]);
        ...
        try {
            $result = $this->pickupRepository->getOrderPerMonthRepo($data['filter']);
        } catch (Exception $e) {
            Log::info($e->getMessage());
            Log::error($e);
            throw new InvalidArgumentException('Gagal mendapatkan order per bulan');
        }
    ...
    /**
     * mendapatkan state pickup (pickup sudah sampai mana)
     */
    public function getPickupStateService($data = [])
    {
        $validator = Validator::make($data, [
            'pickupId'    => 'bail|required'
        ]);
        ...
        try {
            $result = $this->pickupRepository->getPickupStateRepo($data['pickupId']);
        } catch (Exception $e) {
            Log::info($e->getMessage());
            Log::error($e);
            throw new InvalidArgumentException('Gagal mendapatkan data status dokumen pickup');
        }
    ...
    /**
     * mendapatkan data pickup
     */
    public function getPickupByIdService($data = [])
    {
        $validator = Validator::make($data, [
            'pickupId'    => 'bail|required'
        ]);
        ...
        try {
            $result = $this->pickupRepository->getById($data['pickupId']);
        } catch (Exception $e) {
            Log::info($e->getMessage());
            Log::error($e);
            throw new InvalidArgumentException('Gagal mendapatkan data pickup');
