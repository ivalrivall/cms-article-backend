...
class GoogleController extends BaseController
{
    ...
    public function loginGoogle(Request $request)
    {
        ...
        if ($payload['sub'] !== $user->google_id) {
            DB::rollback();
            return $this->sendResponse('Google id tidak valid');
        }

        // validate role
        try {
            $this->roleService->validateRoleLogin($user, 'customer', true);
        } catch (Exception $e) {
            DB::rollback();
            Log::info($e->getMessage());
            Log::error($e);
    ...
    public function registerGoogle(Request $request)
    {
        ...
        try {
            $registeredUser = $this->userService->getByEmailService($payload['email']);
        } catch (Exception $e) {
            DB::rollback();
            Log::info($e->getMessage());
            Log::error($e);
        ...
        if ($registeredUser) {
            if ($registeredUser->google_id == null) {
                return $this->sendError('Mohon maaf email ' .$payload['email']. ' sudah terdaftar, silahkan login');
            }
            return $this->sendError('Mohon maaf email ' .$payload['email']. ' sudah terdaftar, silahkan login menggunakan google');
        }
