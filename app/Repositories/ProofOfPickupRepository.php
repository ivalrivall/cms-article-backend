...
class ProofOfPickupRepository
{
    ...
    public function getSubmittedPickupRepo($data = [])
    {
        ...
        $branchId = $data['branchId'];
        $statusPick = $data['statusPick'];

        $pop = $this->pop->with(['pickup' => function($q) {
        ...
        if (!empty($customer)) {
            $pop = $pop->where('name', 'ilike', '%'.$customer.'%');
        }

        if (!empty($poNumber)) {
            $pop = $pop->whereHas('pickup', function($q) use ($poNumber) {
                $q->where('number', 'ilike', '%'.$poNumber.'%');
            });
        }

        if (!empty($poPickupDate)) {
            $pop = $pop->whereDate('picktime', date($poPickupDate));
        }

        if (!empty($poStatus)) {
            $pop = $pop->where('status', $poStatus);
        }

        if (!empty($poCreatedDate)) {
            $pop = $pop->whereDate('created_at', date($poCreatedDate));
        }

        if (!empty($pickupPlanNumber)) {
            $pop = $pop->whereHas('pickupPlan', function($q) use ($pickupPlanNumber) {
                $q->where('number', 'ilike', '%'.$pickupPlanNumber.'%');
            });
        }

        if (!empty($popNumber)) {
            $pop = $pop->where('number', 'ilike', '%'.$popNumber.'%');
        }

        if (!empty($popDate)) {
            $pop = $pop->whereDate('created_at', date($popDate));
        }

        if (!empty($popStatus)) {
            $pop = $pop->where('status', $popStatus);
        }

        if (!empty($driverPick)) {
            $pop = $pop->where('driver_pick', $driverPick);
        }

        if (!empty($statusPick)) {
            $pop = $pop->where('status_pick', 'ilike', "%$statusPick%");
        }
     ...
     * cancel pop repo
     */
    public function cancelPopRepo($pickupId)
    {
        $pickup = $this->pickup->find($pickupId);
        if ($pickup->shipment_plan_id !== null) {
            throw new InvalidArgumentException('Proof of pickup tidak dapat dibatalkan, karena shipment plan pada order sudah terbuat');
        }
     ...
     * validate order
     */
    public function validateOrder($pickupId)
    {
        $pop = $this->pop->select('id','status')->where('pickup_id', $pickupId)->get();
        if (!$pop) {
            return true;
        }
        if (count($pop) > 0) {
            $statusNotCanceled = collect($pop)->where('status', '!=', 'canceled')->all();
            if (count($statusNotCanceled) !== 0) {
                throw new InvalidArgumentException('Maaf, order ini sudah ter-submit, silahkan pilih order lain.');
            } else {
                return true;
            }
        } else {
    ...
    {
        $pop = $this->pop->find($id);
        if (!$pop) {
            throw new InvalidArgumentException('Maaf, proof of pickup tidak ditemukan');
        }
        return $pop;
    }

    /**
     * check pop have shipment
     */
    public function checkPOPHaveShipment($popId)
    {
        $pop = $this->getById($popId);
        if ($pop->pickup->shipment_plan_id !== null) {
            throw new InvalidArgumentException('Maaf, proof of pickup sudah masuk ke shipment plan, sehingga tidak dapat dibatalkan');
        }
        return $pop;
    }
     ...
     * delete pop
     */
    public function deletePopRepo($popId, $userId)
    {
        $pop = $this->checkPOPHaveShipment($popId);
        $pop->status = 'canceled';
        $pop->updated_by = $userId;
        $pop->save();
        return $pop;
    }
     ...
     * cancel driver_pick
     */
    public function cancelDriverPick($popId)
    {
        $pop = $this->checkPOPHaveShipment($popId);
        $pop->driver_pick = false;
        $pop->save();
        return $pop;
    }

    /**
     * delete pop by pickupId
     */
    public function deleteByPickupId($id, $deletorId)
    {
        $pop = $this->pop->where('pickup_id', $id);
        $pop->update(['deleted_by' => $deletorId]);
        $pop->delete();
        return $pop;
    }
}
