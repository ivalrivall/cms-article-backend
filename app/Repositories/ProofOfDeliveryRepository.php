...
class ProofOfDeliveryRepository
{
     ...
     * update pod repo
     */
    public function updatePodRepo($data = [])
    {
        $pod = $this->pod->where('pickup_id', $data['pickupId'])->update(
            [
                'status_delivery' => $data['statusDelivery'],
                'notes' => $data['notes'],
                'updated_by' => $data['userId'],
                'redelivery_count' => $data['totalRedelivery']
            ]
        );
        return $pod;
    }
    ...
    public function getDetailPickupAdminRepo($data = [])
    {
        $pickup = $this->pickup->select('id','name','phone','picktime','sender_id','receiver_id','shipment_plan_id','status','number')->where('id', $data['pickupId'])->with(
            [
                ...
                'items' => function($q) {
                    $q->select('id','name','pickup_id','unit_count','service_id','weight','volume','price','route_price_id');
                },
                'cost',
                'items.service' => function($q) {
                    $q->select('id','name');
                },
                'items.routePrice' => function($q) {
                    $q->select('id','type');
                },
                'shipmentPlan' => function($q) {
                    $q->select('id','number','driver_id');
                },
                'shipmentPlan.driver' => function($q) {
                    $q->select('id','user_id');
                },
                'shipmentPlan.driver.user' => function($q) {
                    $q->select('id','name');
                },
                'proofOfDelivery' => function($q) {
     ...
     * delete pod repo
     */
    public function deletePodRepo($data = [])
    {
        $pod = $this->pod->where('pickup_id', $data['pickupId']);
        if (!$pod->first()) {
            return true;
        }
        $pod->update(['deleted_by' => $data['userId']]);
        $pod->delete();
        return $pod;
    }
}
